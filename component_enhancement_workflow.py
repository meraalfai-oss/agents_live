...
class ComponentEnhancer:
    ...
    def create_enhanced_version(
        ...
        best_features: Dict[str, List[str]]
            ...
            # Sort by feature importance (simple heuristic: shorter names = more general/important)
            unique_features.sort(key=lambda x: (len(x), x))
            
            ranked_features[feature_type] = unique_features[:10]  # Keep top 10 features
...
if __name__ == "__main__":
    component = Enhanced{category.title().replace("_", "")}()
